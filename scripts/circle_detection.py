...
def callback(data):
    ...
    if len(marker) > 0:
        ...
        print("cX and cY image : {},{} - cX and cY frame : {},{}".format(cx_img,
              cy_img, cx_frm, cy_frm))

        # draw line from centroid image into centroid frame
        cv2.line(modul.img, (cx_img, cy_img),
                 (cx_frm, cy_frm), (255, 255, 255), 4)
        ...
        distance = modul.distance_to_camera(
            19.5, 784.615384615, detected_marker[1][0])

        # print out the result
        cv2.putText(modul.img, "distanece   : {} cm".format(distance),
                    (30, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255, 255, 255), 2)
        cv2.putText(modul.img, "x direction : {} px".format(
            cx_frm-cx_img), (30, 45), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255, 255, 255), 2)
        cv2.putText(modul.img, "y direction : {} px".format(
            cy_frm-cy_img), (30, 65), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255, 255, 255), 2)
